...
public:
	...
	IINaveState* EstadoNormal; //Puntero de interfaz con names 1
	IINaveState* EstadoNokia;
	IINaveState* EstadoAplazado;
	IINaveState* Estado; //ACTUAL

	void InicializarEstados(); //Aqui almaceno cada estado que le ody a la interface IINaveState
	void MoverAliada(float DeltaTime);
	void DispararAliada();
	void CambiarMalla();
	void MostrarEstado(IINaveState* _State);

	IINaveState* GetEstadoNormal(); //Son metodos accesores donde devuelvo un estado  REFERENCE 1
